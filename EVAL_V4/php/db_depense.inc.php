...
class DepenseRepository {
    ...
    public function getDepenseByLibelleOrTag($gid, $libeTag) {
        $result  = null;
        $bdd     = null;

        try {
            $bdd = DBLink::connect2db(MYDB, $message);
            $stmt = $bdd->prepare("SELECT * FROM ".self::TABLE_NAME." WHERE gid =:gid AND (libelle = :libeTag OR tag = :libeTag)");
            $stmt->bindValue(':gid', $gid);
            $stmt->bindValue(':libeTag', $libeTag);
            if ($stmt->execute()){
                $result = $stmt->fetchAll(PDO::FETCH_CLASS, "Depense\Depense");
            }
            $stmt = null;
    ...
    public function getDepenseByMontant($gid, $montMin, $montMax) {
        $result  = null;
        $bdd     = null;

        try {
            $bdd = DBLink::connect2db(MYDB, $message);
            $stmt = $bdd->prepare("SELECT * FROM ".self::TABLE_NAME." WHERE gid =:gid AND (montant BETWEEN :montMin AND :montMax)");
            $stmt->bindValue(':gid', $gid);
            $stmt->bindValue(':montMin', $montMin);
            $stmt->bindValue(':montMax', $montMax);
            if ($stmt->execute()){
                $result = $stmt->fetchAll(PDO::FETCH_CLASS, "Depense\Depense");
            }
            $stmt = null;
    ...
    public function getDepenseByDate($gid, $dateDeb, $dateFin) {
        $result  = null;
        $bdd     = null;

        try {
            $bdd = DBLink::connect2db(MYDB, $message);
            $stmt = $bdd->prepare("SELECT * FROM ".self::TABLE_NAME." WHERE gid =:gid AND (date BETWEEN :dateDeb AND :dateFin)");
            $stmt->bindValue(':gid', $gid);
    ...
    public function getDepenseByMontantAndDate($gid, $montMin, $montMax, $dateDeb, $dateFin) {
        $result  = null;
        $bdd     = null;
        ...
        try {
            $bdd = DBLink::connect2db(MYDB, $message);
            $stmt = $bdd->prepare("SELECT * FROM ".self::TABLE_NAME." WHERE gid = :gid
                        AND (montant BETWEEN :montMin AND :montMax)
    ...
    public function getDepenseByAdvancedSearch($gid, $libeTag, $montMin, $montMax, $dateDeb, $dateFin) {
        $result  = null;
        $bdd     = null;
        ...
        try {
            $bdd = DBLink::connect2db(MYDB, $message);
            $stmt = $bdd->prepare("SELECT * FROM ".self::TABLE_NAME." WHERE gid = :gid
                        AND (libelle = :libeTag OR tag = :libeTag)
                        AND (montant BETWEEN :montMin AND :montMax)
                        AND (date BETWEEN :dateDeb AND :dateFin)");
            $stmt->bindValue(':gid', $gid);
            $stmt->bindValue(':libeTag', $libeTag);
